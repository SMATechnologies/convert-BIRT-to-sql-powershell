        ...
        { 
            ...
            {   
                ...
                else #Send the job add through the API
                {
                    ...
                    SMA_ScheduleAction -url $url -token $token -sname "ADHOC" -jname "RUN REPORT" -jfreq "ONREQUEST" -action "JOB:ADD" -instprops "REPORT=$reportname;FILE=$jsonpath;OUTPUT=$outputpath;EMAIL=$email"  #replaced report with reportname
                }
